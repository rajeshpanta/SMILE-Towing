...
struct DriverSignUpView: View {
    ...
    var body: some View {
        ...
        .padding()
        .navigationTitle("Driver Sign-Up")
            ...
            Group {
                if showTermsPopup {
                    VStack(spacing: 20) {
                        ...
                        ScrollView {
                            Text("""
                            1. Drivers must always ensure the safety of the vehicle and passengers during towing.
                            2. Customers are responsible for providing accurate details about their location and vehicle.
                            3. Payments for services must be completed as per the agreed terms.
                            4. Drivers must adhere to traffic rules and regulations during towing.
                            5. The company is not liable for damages caused by incorrect customer information.
                            6. Customers must inspect their vehicle before and after towing and report any issues immediately.
                            7. Both drivers and customers must treat each other respectfully and professionally.
                            """)
        ...
        Auth.auth().createUser(withEmail: email, password: password) { authResult, error in
            ...
            if let user = authResult?.user {
                ...
                db.collection("users").document(user.uid).setData([
                    ...
                    "lastName": lastName,
                    "email": email,
                    "role": "Driver" // Save the role as "Driver"
                ]) { error in
                    if let error = error {
